# 多端口路由（Routes）— 設定檔格式參考（進階）

> app 已內建完整的路由圖形介面——一般使用只要在「總覽」分頁新增路由即可。
> 本文件是給想**直接編輯設定檔**、或想了解格式的進階使用者。

app 啟動時會讀 `config.json` 裡的 `routes`，替每一條開一個本地端口，各自綁定你指定的
**單一代理，或多層串接（串鏈）**，彼此獨立、同時運作。
