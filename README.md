...
- **斷線保護（Kill-switch）**：分流引擎萬一意外掛掉，會自動以封鎖模式重建虛擬網卡，把受保護的程式擋下來，網域與地區規則同樣受保護。⚠️ **目前不是完全的 fail-closed**：從引擎中止到封鎖生效之間約有 5 秒空窗，這段期間受保護程式的連線會直接出去（[#3](https://github.com/guan4tou2/relay-client/issues/3)）。
...
| 多層串接 | ✅ 每條路由都能 | ❌ | ✅ | ➖ | ✅ 代理群組 | ✅ |
| 指定程式的**斷線保護** | ➖ 崩潰後約 5 秒才封鎖（[#3](https://github.com/guan4tou2/relay-client/issues/3)） | ➖ 有封鎖規則，非 fail-closed | ➖ | ❌ | ➖ 只有全域 | ➖ |
...
- **密碼保護**：伺服器密碼與憑證庫以作業系統的加密儲存（Windows DPAPI／macOS 鑰匙圈／Linux libsecret，經由 Electron `safeStorage`）加密後再寫進設定檔；這台機器不支援加密時會在紀錄裡明講。
- **HTTPS 代理憑證驗證**：預設驗證代理伺服器的 TLS 憑證。使用自簽憑證的代理可在該伺服器設定開啟「略過憑證驗證」（列表會以琥珀色標示）。升級前就存在的 HTTPS 伺服器會先沿用舊的不驗證行為。
- **外部輸入檢查**：匯入檔裡的路由 id／埠號、送進系統設定的埠號、要啟動的程式路徑都會先驗證。
